...
3. **Реализуй по скиллу tdd:** DoD → падающие тесты → минимальная
   реализация → зелёные `./scripts/check` и `./scripts/test`. Для issue-бага
   первый шаг всегда — падающий тест, воспроизводящий баг по шагам из issue;
...
5. **PR — черновиком.** Коммить логическими шагами. `git push -u origin
   <ветка>`, затем `gh pr create --draft` с телом: `Closes #N`, что сделано,
