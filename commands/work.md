...
2. **Подготовь ветку.** Прежде чем создавать новую ветку, проверь, нет
   ...
   'issue-<N>-*'`). Один и тот же слаг, присутствующий и локально, и на
   origin (обычный случай уже запушенной работы), — сведи их одной
   командой, не переключаясь ни в какую ветку: `git fetch origin
   issue-<N>-...:issue-<N>-...`. Она сама делает fast-forward локальной
   ветки до origin (допушили с другой сессии/машины — подтянется) и
   отказывает, когда fast-forward невозможен; текущий HEAD в сверке не
   участвует, main и другие ветки она не трогает. Отказ разбери:
   `git fetch origin issue-<N>-...`, затем `git merge-base --is-ancestor
   origin/issue-<N>-... issue-<N>-...` — origin предок локальной
   (обычные незапушенные локальные коммиты) — расхождения нет, кандидат
   — локальная как есть; иначе история разошлась, это неоднозначность,
   как и разные имена веток с этим префиксом (свои слаги локально и на
   origin, либо несколько веток на одной стороне).
   - **Кандидат один** (после сверки выше) — переключись в него
     ...
     origin/issue-<N>-...`). Не трогай незакоммиченные изменения и не
   ...
   - **Кандидатов несколько или история разошлась** (см. выше) —
