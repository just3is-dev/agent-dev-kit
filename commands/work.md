...
2. **Подготовь ветку.** Обнови main (`git pull`). Прежде чем создавать
   новую ветку, проверь, нет ли уже ветки этой задачи: локально
   (`git branch --list 'issue-<N>-*'`) и на origin (`git ls-remote
   --heads origin 'issue-<N>-*'`). Один и тот же слаг, присутствующий и
   локально, и на origin (обычный случай уже запушенной работы), — это
   один кандидат, не два; разные имена веток с этим префиксом (свои
   слаги локально и на origin, либо несколько веток на одной стороне) —
   разные кандидаты.
   - **Кандидат один** — переключись в него (`git checkout
     issue-<N>-...`; если ветка есть только на origin — заведи локальную,
     отслеживающую её: `git checkout -b issue-<N>-...
     origin/issue-<N>-...`). Не трогай незакоммиченные изменения и не
     переписывай WIP-коммиты — продолжай цикл с текущего состояния
     ветки. Актуализируй её от main по рецепту шага 6 (ниже): гейты
     прогони перед публикацией результата, как там описано. Если по
     этой ветке уже открыт PR (`gh pr list --head issue-<N>-...`) — в
     шаге 5 не создавай второй, используй существующий; PR уже переведён
     в ready — не возвращай его в draft.
   - **Кандидатов несколько** — остановись и спроси пользователя, какую
     ветку продолжать; не выбирай самостоятельно.
   - **Кандидатов нет** — создай `issue-<N>-<слаг>`.
