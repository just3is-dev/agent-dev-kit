...
2. **Подготовь ветку.** Прежде чем создавать новую ветку, проверь, нет
   ...
   - **Кандидат один** — сначала переключись в него
     ...
     (допушили с другой сессии/машины) — ff-подтяжка пройдёт; отказ
     разбери: `git merge-base --is-ancestor origin/issue-<N>-...
     issue-<N>-...` — origin предок локальной (обычные незапушенные
     локальные коммиты) — расхождения нет, работай как есть; иначе
     история разошлась — неоднозначность, стоп (см. ниже); отказ из-за
     незакоммиченных правок в дереве — отложи ff-подтяжку туда же, куда
     и актуализацию (ниже). Не трогай незакоммиченные изменения и не
     ...
     используй существующий; PR уже переведён в ready — не возвращай
     его в draft (что делать при REQUEST_CHANGES по ready-PR — правило
     в шаге 6, по месту действия).
...
6. **Ревью.** Запусти reviewer-агента (agent-dev-kit:reviewer), передав ему
   ...
   /review, она авторизует дополнительный круг). Исключение —
   REQUEST_CHANGES по PR, который уже был переведён в ready
   (переиспользован шагом 2): черновик от случайного merge больше не
   защищает — немедленно останови работу и явно предупреди
   пользователя, не полагаясь на состояние PR и не продолжая круги.
