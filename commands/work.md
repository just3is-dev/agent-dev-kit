...
2. **Подготовь ветку.** Прежде чем создавать новую ветку, проверь, нет
   ...
   origin (обычный случай уже запушенной работы), — кандидат один,
   только если это один и тот же коммит: SHA из `git ls-remote` сверь с
   `git rev-parse issue-<N>-...`. Не совпали — подтяни ветку
   (`git fetch origin issue-<N>-...`) и сравни историю
   (`git merge-base --is-ancestor issue-<N>-... origin/issue-<N>-...` и
   в обратную сторону): origin — потомок локальной (на неё запушили
   позже, с другой сессии/машины) — подтяни fast-forward'ом
   (`git merge --ff-only origin/issue-<N>-...`) и считай кандидатом её;
   локальная — потомок origin (обычные незапушенные локальные коммиты)
   — расхождения нет, кандидат — локальная как есть; ни один не потомок
   другого — история разошлась, это неоднозначность, как и разные имена
   веток с этим префиксом (свои слаги локально и на origin, либо
   несколько веток на одной стороне).
   - **Кандидат один** (после сверки выше) — переключись в него
     ...
     его в draft, но если очередной круг ревью (шаг 6) даст
     REQUEST_CHANGES по уже ready PR — черновик здесь не защищает от
     случайного merge: останови работу и явно предупреди пользователя,
     не полагаясь на состояние PR.
