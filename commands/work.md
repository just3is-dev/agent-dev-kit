...
2. **Подготовь ветку.** Прежде чем создавать новую ветку, проверь, нет
   ли уже ветки этой задачи: локально (`git branch --list
   'issue-<N>-*'`) и на origin (`git ls-remote --heads origin
   'issue-<N>-*'`). Один слаг — локально, на origin или там и там —
   это единственный кандидат; разные имена с этим префиксом (свои слаги
   локально и на origin, либо несколько веток на одной стороне) —
   неоднозначность.
   - **Кандидат один** — сначала переключись в него
     (`git checkout issue-<N>-...`; если он есть только на origin —
     заведи локальную, отслеживающую её: `git fetch origin
     issue-<N>-... && git checkout -b issue-<N>-...
     origin/issue-<N>-...`; checkout отказал из-за незакоммиченных
     правок, конфликтующих с веткой, — остановись и спроси
     пользователя: правки не твои, глушить их stash'ем или `--force`
     нельзя). **Сверка с origin — уже из ветки-кандидата**, когда она
     есть с обеих сторон: `git fetch origin issue-<N>-...`, затем
     `git merge --ff-only origin/issue-<N>-...` — это безопасно именно
     потому, что HEAD стоит на самой ветке (main и другие ветки в
     сверке не участвуют), а fetch формы `ветка:ветка` без checkout
     здесь непригоден — git отказывает обновлять текущую ветку
     (`refusing to fetch into branch ... checked out`). Origin впереди
     (допушили с другой сессии/машины) — ff-подтяжка пройдёт; локальная
     впереди (обычные незапушенные коммиты) — ff скажет «Already up to
     date», это не отказ. Отказ разбирай по его причине из сообщения
     git: «Not possible to fast-forward» — история разошлась —
     неоднозначность, стоп (см. ниже); отказ из-за незакоммиченных
     правок в дереве — отложи ff-подтяжку туда же, куда и актуализацию
     (ниже). Не трогай незакоммиченные изменения и не
     переписывай WIP-коммиты: если в дереве есть незакоммиченные
     правки, рецепт шага 6 (rebase от main) неприменим — на грязном
     дереве он не выполняется, отложи актуализацию до момента, когда
     дерево станет чистым (ближайший коммит цикла шага 3 или сам шаг 6),
     и продолжай цикл прямо с текущего состояния. Когда дерево чистое —
     актуализируй по рецепту шага 6 (ниже); перенос существующих
     коммитов на новую базу — это сам рецепт, а не то «переписывание
     WIP», которого нужно избегать (запрет — на squash/reorder/выброс
     коммитов, не на сам rebase). Если по этой ветке уже открыт PR
     (`gh pr list --head issue-<N>-...`) — в шаге 5 не создавай второй,
     используй существующий. PR уже переведён в ready — прежде чем
     пушить в ветку хоть один новый коммит, верни его в draft
     (`gh pr ready --undo <PR>`): ready — свойство отревьюенной головы,
     новые коммиты её меняют, и без возврата автопилот может смержить
     неотревьюенное; повторный перевод в ready — только шаг 6 после
     нового APPROVE. Не трогаешь ветку (нечего дорабатывать) — ready
     не отменяй.
   - **Кандидатов несколько или история разошлась** (сверка выше) —
     остановись и спроси пользователя, какую ветку продолжать; не
     выбирай самостоятельно.
   - **Кандидатов нет** — обнови main (`git checkout main && git
     pull`), создай `issue-<N>-<слаг>`.
...
3. **Реализуй по скиллу tdd:** DoD → падающие тесты → минимальная
   ...
   пункт, помечается тем же токеном целиком, ровно как он записан в спеке
   (`AC-NNN-K` или, для спек на голой конвенции, `AC-K` — не укорачивай
   префиксованный токен до одного номера критерия; конвенция тегирования —
   skills/tdd); если этот AC в спеке помечен аннотацией `(ждёт #N)` — сними
   её в том же PR: тест появился, «ждать» больше нечего.
...
6. **Ревью.** Запусти reviewer-агента (agent-dev-kit:reviewer), передав ему
   ...
   Порядок «гейты → push», не наоборот: после актуализации сперва
   перегони гейты локально (`./scripts/check`, `./scripts/test`) и
   только потом публикуй результат — если запушить раньше, а гейты
   окажутся красными, PR уже выставляет наружу переписанную
   (rebase/merge) голову, и понадобится второй force-push поверх, чтобы
   её поправить. Гейты красные после актуализации — тот же исход, что и
   конфликт: остановись и позови пользователя (PR остаётся черновиком;
   `--abort` здесь не нужен — актуализация уже применилась, просто её
   результат не проходит собственные гейты). Гейты зелёные — запушь:
   `git push --force-with-lease origin <ветка PR>` после rebase, `git
   push origin <ветка PR>` после merge — и дождись зелёного CI:
   переводить PR в ready без перегона гейтов после актуализации
   запрещено — зелёные проверки отставшей ветки относятся к устаревшему
   состоянию кода. Ветка актуальна, гейты (локальные и CI) зелёные —
   ...
   /review, она авторизует дополнительный круг). Если PR
   почему-либо оказался ready к моменту REQUEST_CHANGES (шаг 2 обязан
   был вернуть его в draft перед новыми коммитами) — сначала верни:
   `gh pr ready --undo <PR>`, затем действуй как обычно; ready без
   действующего APPROVE — нарушенный инвариант, а не рабочее состояние.
