...
1. **Выбор задачи**: `gh issue list --state open` текущего milestone;
   ...
   (issue #158: человек зарезервировал задачу за собой — ставит и снимает
   её сам в любой момент, `gh issue edit N --add-label|--remove-label
   owner:human`), у которого все «Blocked by #N» закрыты. Такую задачу
   пропускай молча: это не застревание и не skip по зависимостям — не
   пиши по ней ничего в журнал, в итоговой сводке не считай её ни
   застрявшей, ни пропущенной (отдельная строка «зарезервировано
   человеком: N» в сводке допустима, но не обязательна). Нет доступных —
   заверши цикл.
...
кроме метки needs-human (метку `owner:human` не ставь и не снимай — это
исключительно решение человека), «доделывать» застрявшие задачи самому — их
