...
3. **Разбор результата** (по фактам из gh, не со слов субагента). После
   каждого решения залогируй итог задачи (журнал — наблюдаемость, не гейт:
   провал записи не блокирует прогон):
   `${CLAUDE_PLUGIN_ROOT}/hooks/scripts/adk-log.sh autopilot-$(date
   +%Y-%m-%d) event=task issue=<N> outcome=<merged|"stuck: причина"|skipped>
   || true` (причина после `stuck:` обычно содержит пробелы — заключай
   `outcome` в кавычки, как в шаге 7 `/work`).
   - **PR ready** → `gh pr merge <PR> --squash --delete-branch` (squash:
     ...
     (`gh pr checks --watch`); после merge — залогируй `outcome=merged`,
     следующая итерация;
   - **PR остался draft или PR нет** → задача застряла: поставь метку
     ...
     "issue #N застрял: <краткая причина>"`), залогируй `outcome="stuck:
     <краткая причина>"`; задачи, зависящие от неё, пропускай до конца
     прогона — на каждую пропущенную залогируй свою запись `outcome=skipped`.
...
итог прогона целиком (тем же файлом, что и записи по задачам —
`autopilot-<дата>` за сегодня):
`${CLAUDE_PLUGIN_ROOT}/hooks/scripts/adk-log.sh autopilot-$(date
+%Y-%m-%d) event=run_finish total=<взято за прогон> merged=<смержено>
