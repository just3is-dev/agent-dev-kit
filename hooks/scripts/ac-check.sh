...
# Голые токены (без префикса спеки AC-<NNN>-<K>) — устаревшая, но поддер-
# живаемая конвенция: не блокируем, только предупреждаем один раз за прогон.
# Число, а не список токенов, — конкретные номера в этом выводе (AC-101,
# AC-3, ...) неотличимы от токенов в остальном выводе гейта (списке
# непокрытых), поэтому перечисление засоряло бы grep-проверки по выводу.
bare_count=0
for ac in "${acs[@]}"; do
  case "$ac" in
    AC-*-*) ;;
