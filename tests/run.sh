...
# ── issue #159: /work шаг 2 переиспользует существующую ветку issue-N ───────
# Смешанный сценарий «человек начал — агент доделывает» упирался в то, что
# шаг 2 всегда создавал ветку заново, плодя параллельные ветки по одному
# issue. Срез шага 2 должен называть: проверку существующей ветки локально
# и на origin, правило «несколько кандидатов — спроси человека, не выбирай
# сам», переиспользование уже открытого PR (в т.ч. ready — не откатывать в
# draft), актуализацию найденной ветки по рецепту шага 6 (ссылкой, а не
# пересказом самого рецепта), и что незакоммиченные правки/WIP-коммиты не
# трогаются.
work_step2=$(md_section "$WORKMD" '^2\. \*\*' '^3\. \*\*')
...
assert_contains "issue #159: work.md шаг 2 — новые коммиты в переиспользованный ready-PR только после возврата в draft (круг 4 ревью PR #169, блокер: инвариант ready⟺APPROVE)" "$work_step2" 'верни его в draft'
assert_contains "issue #159: work.md шаг 2 называет команду возврата (gh pr ready --undo)" "$work_step2" 'gh pr ready --undo'
assert_contains "issue #159: work.md шаг 2 — ready без доработки не отменяется" "$work_step2" 'ready не отменяй'
...
work_step2_nospace=$(printf '%s' "$work_step2" | tr -d ' ')
assert_not_contains "issue #159: work.md шаг 2 — сверка не использует fetch формы ветка:ветка (круг 3 ревью PR #169, блокер: refusing to fetch into checked out; страж по бес-пробельной нормализации, круг 4)" "$work_step2_nospace" ':issue-<N>-'
...
assert_contains "issue #159: work.md шаг 2 разбирает отказ ff по причине из сообщения git (круг 4 ревью PR #169, важно: мёртвая ветка merge-base убрана)" "$work_step2" 'Not possible to fast-forward'
assert_not_contains "issue #159: work.md шаг 2 не использует merge-base в разборе отказа (недостижимая ветка, круг 4)" "$work_step2" 'merge-base'
assert_contains "issue #159: work.md шаг 6 — сторож называет команду возврата в draft (круг 4 ревью PR #169)" "$work_step6" 'gh pr ready --undo'
...
# Репро механизма (полевой сигнал из тела issue): approved-спека без
# тестов и без аннотаций, одна в проекте — уже красная для ac-check. Именно
# в это состояние спека попадает, если её мержить сразу после апрува (как
# сейчас велит /spec), не дожидаясь /plan.
S172="$TMP/issue-172-window"
mkdir -p "$S172/docs/specs" "$S172/tests"
write_ac_spec "$S172/docs/specs/001-x.md" "approved" "- [ ] AC-201: первый критерий
- [ ] AC-202: второй критерий"
: > "$S172/tests/run.sh"
"$HOOKS/ac-check.sh" "$S172" >/dev/null 2>&1
assert_exit "issue #172: репро — approved-спека без тестов и без аннотаций красная для ac-check" 1 $?

# Тот же файл с аннотациями (то, что /plan обязан успеть поставить ДО того,
# как спека попадёт в main) — красного состояния уже нет.
write_ac_spec "$S172/docs/specs/001-x.md" "approved" "- [ ] AC-201 (ждёт #501): первый критерий
- [ ] AC-202 (ждёт #501): второй критерий"
"$HOOKS/ac-check.sh" "$S172" >/dev/null 2>&1
assert_exit "issue #172: та же спека с аннотациями «ждёт #N» — не красная" 0 $?

# Вывод: единственный способ никогда не показывать main в первом состоянии —
# не мержить спеку до того, как аннотации у неё уже есть (issue #172,
# решение — поменять порядок команд, а не сам ac-check.sh: см. doc-тесты
# ниже commands/spec.md и commands/plan.md).
...
# Якорь на смысл («гейт не видит новый стек»), а не на пунктуацию вокруг
# перечня гейтов (обратные кавычки и т.п.) — issue #121.
check_ac_doc "issue #81" "reviewer: проверка видимости нового стека гейтам проекта" \
  "$KIT/agents/reviewer.md" "невидимый гейту, проходит ревью бесшумно"
...
# ── Обновление копии scripts/ac-check в уже инициализированных проектах
# (issue #56, хвост ревью PR #55): project-init копирует хелпер один раз,
...
check_ac_doc "issue #166" "спека 004: docs/contract.md — единственный источник перечня, спека его не дублирует" \
  "$KIT/docs/specs/004-plugin-versioning.md" 'он единственный источник, спека перечень не дублирует'
assert_not_contains "issue #166: спека 004 не содержит собственного перечня файлов плагина (инвариант против дрейфа со contract.md, круг 2 ревью PR #168)" \
