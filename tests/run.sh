...
check_ac_doc "issue #56" "project-init.md: повторный запуск на существующем проекте безусловно обновляет scripts/ac-check" \
  "$PI_MD" "перезаписывай \`scripts/ac-check\` безусловно"
check_ac_doc "issue #56" "contract.md: механизм обновления копии описан однозначно" \
  "$KIT/docs/contract.md" "перезаписывает \`scripts/ac-check\` свежей версией"
check_ac_doc "issue #56" "contract.md: прогон на проекте со старой копией описан" \
  "$KIT/docs/contract.md" "инициализирован до issue #54"
