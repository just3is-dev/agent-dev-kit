...
Хуки (`bash-guard`, `pr-title-check`) ищут `<корень>` не безусловно в
git-toplevel: проект может быть подкаталогом более крупного репозитория
со своим `adk.config.json` (`adk_hook_config_root`, `lib/config.sh`,
issue #78). Кандидаты по порядку — берётся первый, где файл реально
существует: cwd самой команды буквально (явный `cd` в её начале, иначе
cwd сессии из payload хука) → git toplevel именно этой cwd →
`CLAUDE_PROJECT_DIR` буквально → git toplevel команды безусловно (этот
последний — без проверки наличия файла, как и раньше: так
мейнстрим-раскладка «конфиг на корне репозитория» не меняется). Git
toplevel команды идёт раньше `CLAUDE_PROJECT_DIR`, а не наоборот: сессия
может быть открыта в одном проекте, а конкретная команда — идти из
подкаталога совсем другого репозитория со своим, более строгим конфигом
(например `policies.merge=human-only`) — репозиторий команды обязан
выигрывать у корня сессии, если у него вообще есть свой файл.
