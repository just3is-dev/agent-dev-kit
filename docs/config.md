...
существует: cwd самой команды (явный `cd` в её начале, иначе cwd сессии
из payload хука) → `CLAUDE_PROJECT_DIR` → git toplevel команды (этот
последний — безусловно, без проверки наличия файла, как и раньше: так
мейнстрим-раскладка «конфиг на корне репозитория» не меняется).
